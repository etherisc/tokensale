...
contract TokenInterface {
   ...
   *
   */
  function allowance(address owner, address spender)  returns (uint256 remaining);
...
contract HackerGold is StandardToken {
   ...
   */
  function getPrice()  returns (uint result) {
   ...
   */
  function getTotalSupply()  returns (uint result) {
    return totalSupplyVar;
  }
   ...
   */
  function getNow()  returns (uint result) {
    return now;
  }
   ...
   */
  function getTotalValue()  returns (uint result) {
    return totalValue;
  }
}
...
contract EventInfo{
   ...
   */
  function getEventStart()  returns (uint result){
    return eventStart;
  }
   ...
   */
  function getEventEnd()  returns (uint result){
    return eventEnd;
  }
   ...
   *                  event starts
   */
  function getVotingStart()  returns (uint result){
   ...
   */
  function getTradingStart()  returns (uint result){
    return eventStart+ T_1_WEEK;
  }
...
contract DSTContract is StandardToken{
  ...
  function votingRightsOf(address _owner)  returns (uint256 result) {
    result = votingRights[_owner];
  }

  function getPreferedQtySold()  returns (uint result){
  ...
  function getHKGOwned()  returns (uint result){
    return hackerGold.balanceOf(this);
  }

  function getEtherValue()  returns (uint result){
    return this.balance;
  }

  function getExecutive()  returns (address result){
    return executive;
  }

  function getHKGPrice()  returns (uint result){
    return hkgPrice;
  }

  function getEtherPrice()  returns (uint result){
    return etherPrice;
  }

  function getDSTName()  returns(string result){
    return name;
  }

  function getDSTNameBytes()  returns(bytes32 result){
    return convert(name);
  }

  function getDSTSymbol()  returns(string result){
    return symbol;
  }

  function getDSTSymbolBytes()  returns(bytes32 result){
    return convert(symbol);
  }

  function getAddress()  returns (address result) {
    return this;
  }
  ...
  function getCollectedEther()  returns (uint results) {
    return collectedEther;
  }

  function getCounterProposals()  returns (uint result){
    return counterProposals;
  }

  function getProposalIdByIndex(uint i)  returns (bytes32 result){
    return listProposals[i].id;
  }

  function getProposalObjectionByIndex(uint i)  returns (uint result){
    return listProposals[i].votesObjecting;
  }

  function getProposalValueByIndex(uint i)  returns (uint result){
    return listProposals[i].value;
  }

  function getCurrentImpeachmentUrlDetails()  returns (string result){
    return lastImpeachmentProposal.urlDetails;
  }


  function getCurrentImpeachmentVotesSupporting()  returns (uint result){
